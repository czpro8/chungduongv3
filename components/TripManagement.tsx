...
import { 
  ClipboardList, Search, Clock, ArrowUpDown, Play, CheckCircle2, XCircle, Loader2, ArrowRight, User, Car, History, Timer, X, AlertCircle, ChevronDown, Check, Phone, Calendar
} from 'lucide-react';
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      <button 
        ...
        className={`w-full flex items-center justify-between px-2.5 py-2 rounded-xl border transition-all duration-300 relative z-20 ${currentStatus.style} ${isOpen ? 'ring-2 ring-indigo-100 border-indigo-400 shadow-sm' : 'hover:brightness-95'}`}
      >
            ...
            {filteredOptions.length > 0 ? filteredOptions.map((opt) => (
              <button 
                ...
                onMouseDown={(e) => { 
                  ...
                  onChange(opt.value); setIsOpen(false); setStatusSearch('');
                }}
  ...
  const displayTrips = useMemo(() => {
    ...
    let filtered = trips.filter(trip => {
      ...
      const matchesSearch = removeAccents(trip.origin_name).includes(searchNormalized) || 
                            ...
                            (trip.trip_code && removeAccents(trip.trip_code).includes(searchNormalized)) ||
          ...
          <table className="w-full text-left table-fixed min-w-[1250px]">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                <SortHeader label="Thông tin chuyến" sortKey="created_at" width="15%" />
                <SortHeader label="Phương tiện" sortKey="vehicle_info" width="18%" />
                <SortHeader label="Trạng thái" sortKey="status" width="17%" textAlign="text-center" />
                <SortHeader label="Lộ trình" sortKey="origin_name" width="21%" />
                <SortHeader label="Lịch trình" sortKey="departure_time" width="17%" />
                <SortHeader label="Giá" sortKey="price" width="12%" textAlign="text-right" />
              </tr>
            </thead>
            {loading ? (
                  ...
                  return (
                    <tr key={trip.id} className="hover:bg-slate-50/30 transition-colors">
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-2">
                          <div className="min-w-0 flex-1">
                            <p className="text-[10px] font-bold text-slate-800 truncate leading-tight">{originInfo.huyen}</p>
                            <p className="text-[8px] font-bold text-slate-400 truncate">{originInfo.tinh}</p>
                          </div>
                          <ArrowRight size={8} className="text-slate-300 shrink-0" />
                          <div className="min-w-0 flex-1">
                            <p className="text-[10px] font-bold text-emerald-600 truncate leading-tight">{destInfo.huyen}</p>
                            <p className="text-[8px] font-bold text-slate-400 truncate">{destInfo.tinh}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-4">
                        ...
                        <div className="text-[9px] font-bold text-slate-400 mt-0.5">{depDate}</div>
                      </td>
                      ...
                      <td className="px-4 py-4 text-right pr-6">
                        ...
                        <div className="flex flex-col items-end mt-1.5 gap-1">
                          <span className="text-[8px] font-bold text-slate-400">{trip.seats - trip.available_seats}/{trip.seats} ghế</span>
