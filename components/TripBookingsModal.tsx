...
import { Trip, Booking, TripStatus } from '../types'; // Import TripStatus
...
const TripBookingsModal: React.FC<TripBookingsModalProps> = ({ trip, bookings, isOpen, onClose, onRefresh }) => {
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-4xl rounded-[32px] md:rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 h-[90vh] flex flex-col relative">
        <button 
          onClick={onClose} 
          className="absolute top-4 right-4 md:-top-4 md:-right-4 w-10 h-10 md:w-12 md:h-12 bg-rose-500 text-white rounded-full flex items-center justify-center shadow-lg shadow-rose-500/30 hover:rotate-90 hover:bg-rose-600 transition-all duration-300 z-[160] border-2 border-white"
        >
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
             <div className="flex items-center gap-6 w-full md:w-auto justify-center md:justify-start">
                <div className="text-center">
                   <p className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Tổng ghế đã đặt</p>
                   <p className="text-lg md:text-xl font-black text-slate-900 font-outfit">
                      {bookings.reduce((sum, b) => sum + (b.status === 'CONFIRMED' || b.status === 'PENDING' ? b.seats_booked : 0), 0)}/{trip.seats} Ghế
                   </p>
                </div>
                <div className="w-px h-8 bg-slate-200"></div>
                <div className="text-center">
                   <p className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Dự kiến thu hộ</p>
